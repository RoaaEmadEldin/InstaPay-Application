...
public class WalletTransfer extends Transferring
{
    ...
    {
        if (transferredMoney > (double) WalletAPI.getUser("id", source).get("balance"))
        {
    ...
    {
        double srcCurrentBalance = (double) WalletAPI.getUser("id", source).get("balance");
        srcCurrentBalance -= transferredMoney;
        double destCurrentBalance = (double) WalletAPI.getUser("id", dest).get("balance");
